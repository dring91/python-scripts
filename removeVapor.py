...
def main():
  ...
  with open(trajFile,'r') as inp:
    for n in range(nFrames):
      ...
      time, frame = readFrame(inp)
      particles = frame[frame[:,0] >= 3]
      polymers = frame[frame[:,0] == 1][:,3]

      # make box
      box = np.zeros((3,2))
      box[:,0] = particles[:,1:].min(0)
      box[:,1] = particles[:,1:].max(0)

      particles = particles[:,1:].reshape((nPart,nAtoms,nDim))

      polymers -= box[2,0]
      polymers /= (box[2,1] - box[2,0])
      density = makeHistogram(polymers,
                              nSlices,
                              (box[2,1] - box[2,0])/(polymers.min() - box[2,0]))
